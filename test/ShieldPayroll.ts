...
describe("ShieldPayroll — End-to-End Flow", function () {
  ...
  describe("Add Employees", function () {
    ...
    it("employer adds Bob with a different encrypted salary", async function () {
      ...
      await (await payroll.connect(accounts.employer).addEmployee(accounts.alice.address, "Alice", encA.handles[0], encA.inputProof)).wait();

      const encB = await encryptAmount(BOB_SALARY, payrollAddress, accounts.employer.address);
      await (await payroll.connect(accounts.employer).addEmployee(accounts.bob.address, "Bob", encB.handles[0], encB.inputProof)).wait();
    ...
    it("cannot add the same employee twice", async function () {
      const enc = await encryptAmount(ALICE_SALARY, payrollAddress, accounts.employer.address);
      await (await payroll.connect(accounts.employer).addEmployee(accounts.alice.address, "Alice", enc.handles[0], enc.inputProof)).wait();

      const enc2 = await encryptAmount(ALICE_SALARY, payrollAddress, accounts.employer.address);
      await expect(
        payroll.connect(accounts.employer).addEmployee(accounts.alice.address, "Alice", enc2.handles[0], enc2.inputProof),
    ...
    it("non-employer cannot add employees", async function () {
      ...
      await expect(
        payroll.connect(accounts.alice).addEmployee.staticCall(accounts.charlie.address, "Charlie", ethers.ZeroHash as any, "0x"),
      ).to.be.revertedWithCustomError(payroll, "NotEmployer");
    });
    ...
    it("employer can batch-add multiple employees", async function () {
      ...
      const tx = await payroll.connect(accounts.employer).batchAddEmployees(
  ...
  describe("Salary Decryption (ACL)", function () {
    ...
    it("non-employee cannot call getMyEncryptedSalary", async function () {
      await expect(
        payroll.connect(accounts.charlie).getMyEncryptedSalary(),
  ...
  describe("Execute Payroll — Cycle 1", function () {
    ...
    it("non-employer cannot execute payroll", async function () {
      await expect(
        payroll.connect(accounts.alice).executePayroll(),
      ).to.be.revertedWithCustomError(payroll, "NotEmployer");
    });
  ...
  describe("Payment History — Employee Verification", function () {
    ...
    it("non-employee cannot query payment history", async function () {
      await expect(
        payroll.connect(accounts.charlie).getMyPaymentForCycle(1),
      ).to.be.revertedWithCustomError(payroll, "NotEmployee");
    });
  });
  ...
  describe("Update Salary", function () {
    ...
    it("employer updates Alice's salary and she decrypts new value", async function () {
      ...
      await (await payroll.connect(accounts.employer).updateSalary(accounts.alice.address, enc.handles[0], enc.inputProof)).wait();
  ...
  describe("Multi-Cycle Payroll", function () {
    ...
    beforeEach(async function () {
      ...
      await (await payroll.connect(accounts.employer).updateSalary(accounts.alice.address, encRaise.handles[0], encRaise.inputProof)).wait();
  ...
  describe("Access Control", function () {
    ...
    it("inactive employee cannot query salary", async function () {
      ...
      await expect(
        payroll.connect(accounts.alice).getMyEncryptedSalary(),
      ).to.be.revertedWithCustomError(payroll, "NotEmployee");
    });

    it("getEncryptedTreasuryBalance reverts for non-employer", async function () {
      await expect(
        payroll.connect(accounts.alice).getEncryptedTreasuryBalance(),
      ).to.be.revertedWithCustomError(payroll, "NotEmployer");
    });
  });
});
